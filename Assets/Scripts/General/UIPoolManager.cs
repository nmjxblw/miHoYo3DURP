...
public class UIPoolManager : MonoBehaviour
{
    ...
    public static GameObject Release(string name, Vector3 pos = default(Vector3))
    {
        if (pos == default(Vector3)) pos = Vector3.zero;
        foreach (var go in dictionary.Keys.ToList())
        {
            if (go.name == name)
                return Release(go, pos);
        }
        return null;
    }
}
